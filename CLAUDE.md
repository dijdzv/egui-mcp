...
| Type | 説明 | バージョン | リリーストリガー |
|------|------|-----------|------------------|
| `feat` | 新機能 | MINOR (0.x.0 → 0.x+1.0) | ✅ Yes |
| `fix` | バグ修正 | PATCH (0.0.x → 0.0.x+1) | ✅ Yes |
| `perf` | パフォーマンス改善 | PATCH | ✅ Yes |
| `refactor` | リファクタリング（機能変更なし） | - | ✅ Yes |
| `test` | テスト追加・修正 | - | ✅ Yes |
| `docs` | ドキュメントのみ | - | ❌ No |
| `style` | フォーマット変更（コード動作に影響なし） | - | ❌ No |
| `chore` | ビルド、依存関係など | - | ❌ No |
| `ci` | CI/CD 設定変更 | - | ❌ No |
| `build` | ビルドシステム変更 | - | ❌ No |

> **Note**: リリーストリガーが ✅ のコミットのみが release-plz による Release PR 作成の対象になります。
